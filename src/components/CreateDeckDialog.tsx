...
export function CreateDeckDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const reset = () => {
    ...
    setCreating(false); setCreatedDeckId(null);
  };
  ...
  const handleAddExisting = async () => {
    if (!name.trim() || busy) return;
    setCreating(true);
    try { const deck = await createDeck(name.trim(), description.trim() || undefined); setCreatedDeckId(deck.id); setPickerOpen(true); }
    finally { setCreating(false); }
  };
  ...
  const handleImportPdf = async () => {
    if (!name.trim() || busy) return;
    setCreating(true);
    try { const deck = await createDeck(name.trim(), description.trim() || undefined); setCreatedDeckId(deck.id); setPdfOpen(true); }
    finally { setCreating(false); }
  };
      ...
      <Dialog open={open} onClose={busy ? undefined : onClose} maxWidth="xs" fullWidth slotProps={{ paper: { sx: dialogPaperSx } }}>
        ...
        <DialogContent sx={{ pt: "16px !important" }}>
          ...
          <TextField label="Description (optional)" value={description} onChange={(e) => setDescription(e.target.value)}
            fullWidth multiline rows={2} disabled={busy} sx={{ mb: 2.5 }} />
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          ...
          <Button
            ...
            startIcon={busy ? <CircularProgress size={13} color="inherit" /> : canGenerate ? <AutoAwesomeIcon sx={{ fontSize: 14 }} /> : undefined}
            sx={{
