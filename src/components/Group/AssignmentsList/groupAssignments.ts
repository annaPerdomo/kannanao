 ...
 * the same handout listed thirty times. The goal fields are part of the key so
 * two handouts of the same deck with different goals stay separate rows — a
 * batch edit must never stamp one member's goal onto another handout's copies.
 * Unfinished batches lead, then the nearest deadline.
 */
export function groupAssignments(assignments: Assignment[]): AssignmentBatch[] {
  const batches = new Map<string, AssignmentBatch>();

  for (const a of assignments) {
    const key = `${a.deck_id}::${a.due_date ?? ''}::${a.required_accuracy ?? ''}::${a.required_mode ?? ''}`;
