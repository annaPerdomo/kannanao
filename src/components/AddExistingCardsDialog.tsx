...
import {
  Button, Typography, TextField, InputAdornment,
  Checkbox, Box, CircularProgress, Divider,
} from '@mui/material';
...
function CardThumbnail({ card }: { card: Flashcard }) {
  ...
  return card.imageUrl ? (
    <Box
      component="img" src={card.imageUrl} alt={card.word}
      sx={{
        width: 44, height: 44, borderRadius: '8px', objectFit: 'cover', flexShrink: 0,
        border: `1px solid ${alpha(brand[300], 0.3)}`, bgcolor: brand[50],
      }}
    />
  ) : (
    <Box sx={{
...
export function AddExistingCardsDialog({ open, onClose, targetDeckId, userId, onConfirm }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([loadAllCards(), loadDecks(userId)]).then(([cards, decks]) => {
      ...
      decks.forEach((d: Deck) => { map[d.id] = d.name; });
      setDeckMap(map); setLoading(false);
    });
  ...
  const filtered = useMemo(() => {
    ...
    return allCards.filter((c) =>
      c.word.toLowerCase().includes(q) || c.reading.toLowerCase().includes(q) || c.meaning.toLowerCase().includes(q));
  }, [allCards, search]);
  ...
  const handleConfirm = async () => {
    ...
    setSaving(true); await onConfirm(chosenCards); setSaving(false); onClose();
  };
    ...
    <StyledDialog
      ...
      actions={
        <>
          ...
          <Button
            onClick={handleConfirm} disabled={selected.size === 0 || saving} variant="contained"
            sx={{
              bgcolor: brand[700], color: brand[50], textTransform: 'none', borderRadius: 6, px: 2.5,
          ...
          >
            ...
            {saving ? 'Copying…' : `Copy ${selected.size > 0 ? selected.size : ''} card${selected.size !== 1 ? 's' : ''}`}
          </Button>
        </>
      }
    >
      ...
      {!loading && filtered.length > 0 && (
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.5 }}>
          <Typography variant="caption" color="text.secondary">
            {selected.size > 0 ? `${selected.size} selected` : `${filtered.length} card${filtered.length !== 1 ? 's' : ''}`}
          </Typography>
          <Button size="small" onClick={toggleAll}
            sx={{ textTransform: 'none', fontSize: '0.75rem', color: brand[700], p: 0, minWidth: 0 }}
          >
      ...
      {loading ? (
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, overflowY: 'auto', maxHeight: 'calc(80vh - 220px)', pr: 0.5 }}>
          {filtered.map((card) => {
            const isChecked = selected.has(card.id);
            return (
              <Box
                key={card.id} onClick={() => toggle(card.id)}
                sx={{
              ...
              >
                <Checkbox checked={isChecked} size="small" disableRipple
                  sx={{ p: 0, color: alpha(brand[700], 0.35), '&.Mui-checked': { color: brand[700] } }}
                />
