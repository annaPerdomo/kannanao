...
import { DAY_LABELS, DAY_INDEX_TO_JS } from './helpers';
...
export function FrequencyPicker({ value, onChange, repeatUntilDone, onRepeatUntilDoneChange, showRepeatUntilDone = true }: FrequencyPickerProps) {
  const theme = useTheme();
  const { brand, accent } = theme.palette;

  const presets = [
    ...(showRepeatUntilDone ? [{ label: 'Until done', isUntilDone: true, days: [] as number[] }] : []),
          ...
          {DAY_LABELS.map((label, i) => {
            ...
            return (
              <Box
                ...
                sx={{
                  height: 28, px: 0.85, borderRadius: 2,
                  border: '2px solid',
