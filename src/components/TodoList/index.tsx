...
export function TodoList({ onXpEarned }: TodoListProps) {
  ...
  useEffect(() => {
    const stored = parseJson<CalendarEntry[]>(
      localStorage.getItem(CALENDAR_ENTRIES_KEY),
      [],
    );
  ...
  const completedCount = todosForDay.filter((t) =>
    isCompletedOnDate(t, activeDateISO),
  ).length;
  ...
  const dayDetailPanel = (
    <Stack spacing={1.5}>
      ...
      {view === "month" && (
        ...
        >
          <Typography
            sx={{ fontWeight: 800, fontSize: "0.85rem", color: brand[700] }}
          >
            📅 {activeDateLabel}
          </Typography>
          {totalCount > 0 && (
      ...
      <Stack spacing={1} useFlexGap>
        {/* Header + progress bar grouped so Stack gap doesn't affect their spacing */}
        <Box>
          <Typography
            sx={{
              fontSize: "0.72rem",
        ...
        <Collapse in={!!celebration}>
          ...
          >
            <Typography
              sx={{ fontWeight: 800, fontSize: "0.88rem", color: brand[700] }}
            >
        ...
        {loading ? (
          ...
          <Stack spacing={0.75}>
            {[
              ...todosForDay.filter(
                (t) => !isCompletedOnDate(t, activeDateISO),
              ),
