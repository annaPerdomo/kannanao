...
export function TodoList({ onXpEarned }: TodoListProps) {
  ...
  useEffect(() => { localStorage.setItem(CALENDAR_ENTRIES_KEY, JSON.stringify(entries)); }, [entries]);
  ...
  const handleAddEntryType = useCallback(async (name: string, emoji: string, color: string) => addEntryType(name, emoji, color), [addEntryType]);
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box
        sx={{
          width: "100%", maxWidth: "100%", mx: "auto",
          ...
          boxShadow: { xs: "none", sm: `0 12px 40px ${alpha(brand[300], 0.28)}, 0 4px 16px ${alpha(accent[200], 0.18)}, inset 0 1px 0 rgba(255,255,255,0.7)` },
          overflow: "hidden",
        }}
      >
        <Box sx={{ background: `linear-gradient(135deg, ${alpha(brand[400], 0.18)} 0%, ${alpha(accent[300], 0.22)} 100%)`, px: { xs: 1.5, sm: 2.5 }, pt: 2, pb: 1.25, borderBottom: `1.5px solid ${alpha(brand[300], 0.12)}` }}>
          <TodoHeader view={view} onViewChange={setView} streak={streak} brandPalette={brand} accentPalette={accent} />
          {view === "week" && (
            <WeekStrip weekDates={weekDates} selectedDayIndex={selectedDayIndex} onSelectDay={setSelectedDayIndex} weekOffset={weekOffset} onWeekChange={setWeekOffset} todos={todos} entries={entries} />
          )}
        </Box>
          ...
          {view === "month" ? (
            <Box sx={{ display: "flex", gap: 2, alignItems: "flex-start" }}>
              <Box sx={{ flex: "0 0 52%", minWidth: 0 }}>
                <MonthCalendar todos={todos} entries={entries} entryTypes={allEntryTypes} monthOffset={monthOffset} onMonthChange={setMonthOffset} selectedDateISO={monthSelectedDateISO} onSelectDate={setMonthSelectedDateISO} />
              </Box>
              <Divider orientation="vertical" flexItem sx={{ borderColor: alpha(brand[200], 0.35) }} />
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <DayDetailPanel
                  view={view} activeDateLabel={activeDateLabel} activeDateISO={activeDateISO} isPastDate={isPastDate}
                  todosForDay={todosForDay} completedCount={completedCount} totalCount={totalCount}
        ...
        <EditTodoDialog open={!!editingTodo} onClose={() => setEditingTodo(null)} todo={editingTodo} onSave={handleSaveAdvancedEdit} />
      </Box>
