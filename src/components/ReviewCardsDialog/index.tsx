    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 1.5 }}>
          <Box>
            <Typography sx={{ fontSize: '1.15rem', fontWeight: 900, color: brand[800], lineHeight: 1.2, mb: 0.4 }}>
              📋 Review Cards
            </Typography>
          ...
          <IconButton
            ...
            sx={{ width: 28, height: 28, color: alpha(brand[700], 0.4), '&:hover': { bgcolor: alpha(brand[300], 0.2), color: brand[700] } }}
          >
        ...
        {/* Global controls bar */}
        <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1.5, bgcolor: alpha('#fff', 0.6), border: `1px solid ${alpha(brand[300], 0.25)}`, borderRadius: '10px', px: 1.5, py: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
            <Typography sx={{ fontSize: '0.6rem', fontWeight: 700, color: alpha(brand[700], 0.6), whiteSpace: 'nowrap' }}>
              Set main view mode for all cards:
            </Typography>
            <ToggleButtonGroup value={allViewMode} exclusive size="small" onChange={(_, v) => { if (v) handleSetAllViewMode(v); }} sx={toggleSx}>
      ...
      <DialogContent sx={{ px: 2, pt: 2, pb: 0, display: 'flex', flexDirection: 'column', gap: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
        {cards.length === 0 && (
          <Typography sx={{ textAlign: 'center', color: alpha(brand[700], 0.6), fontSize: '0.85rem', py: 4 }}>
      ...
      {/* Footer */}
      <Box sx={{ px: 2.5, py: 2, borderTop: `1.5px solid ${alpha(brand[300], 0.2)}`, display: 'flex', gap: 1.5, justifyContent: 'flex-end', background: `linear-gradient(0deg, ${brand[50]} 0%, transparent 100%)` }}>
        <Button
          variant="outlined" onClick={onClose}
          sx={{ borderRadius: '10px', fontWeight: 700, fontSize: '0.82rem', textTransform: 'none', borderColor: alpha(brand[300], 0.5), color: brand[700], '&:hover': { borderColor: brand[400], bgcolor: alpha(brand[300], 0.06) } }}
        >
          Cancel
        </Button>
        <Button
          ...
          sx={{
            ...
            background: cards.length > 0 ? `linear-gradient(135deg, ${brand[400]} 0%, ${brand[500]} 50%, ${accent[500]} 100%)` : undefined,
