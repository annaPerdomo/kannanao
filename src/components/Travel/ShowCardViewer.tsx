...
function ShowCardDisplay({ card, expanded }: { card: ShowCard; expanded: boolean }) {
...
export function ShowCardViewer() {
  ...
  const { generating, error, generateCard, filterByCategory } = useShowCards();
  ...
  const handleGenerate = async () => {
    ...
    const result = await generateCard(customInput.trim());
    if (result) {
      setCustomInput('');
      setShowCreate(false);
    ...
    <Container maxWidth="sm" sx={{ py: 4 }}>
      <Stack spacing={3}>
        ...
        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
          Hold your phone up to show these cards to Japanese people when you can&apos;t speak. The
          large Japanese text is meant for them to read.
        </Typography>
        ...
        <Stack spacing={2}>
          {filteredCards.map((card) => (
            <Box
              ...
              sx={{ cursor: 'pointer' }}
            >
              <ShowCardDisplay card={card} expanded={expandedId === card.id} />
            </Box>
          ))}
        </Stack>
