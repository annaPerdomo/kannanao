...
import {
  Button, Typography, TextField, Box, CircularProgress, IconButton, Divider,
} from '@mui/material';
...
interface Props { open: boolean; onClose: () => void; deckId: string; deckName: string; }
...
export function ShareDeckDialog({ open, onClose, deckId, deckName }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    setUsername(''); setError(null); setLoading(true);
    dbGetDeckShares(deckId).then((s) => { setShares(s); setLoading(false); });
  }, [open, deckId]);
  ...
  const handleShare = async () => {
    ...
    if (error) { setError(error); setSharing(false); return; }
    const updated = await dbGetDeckShares(deckId);
    setShares(updated); setUsername(''); setSharing(false);
  };
    ...
    >
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
        <TextField
          ...
          size="small" fullWidth disabled={sharing}
        />
        <Button
          onClick={handleShare} disabled={sharing || !username.trim()} variant="contained"
          sx={{
            bgcolor: accent[600], color: '#fff', textTransform: 'none', borderRadius: 6,
            whiteSpace: 'nowrap', flexShrink: 0,
            '&:hover': { bgcolor: accent[700] },
            '&:disabled': { bgcolor: alpha(accent[600], 0.25) },
          }}
          startIcon={sharing ? <CircularProgress size={14} color="inherit" /> : <PersonAddAltIcon fontSize="small" />}
        >
          Share
        </Button>
      </Box>
      ...
      {error && (
        <Typography variant="caption" sx={{ color: 'error.main', mt: 0.75, display: 'block' }}>{error}</Typography>
      )}
          ...
          {shares.map((share) => (
            <Box key={share.id} sx={{
              ...
              <IconButton size="small" onClick={() => handleRemove(share.id)} aria-label="Remove share"
                sx={{ color: alpha('#dc2626', 0.5), '&:hover': { color: '#dc2626', bgcolor: 'rgba(220,38,38,0.08)' } }}>
