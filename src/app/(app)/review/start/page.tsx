...
export default function PracticeStartPage() {
  ...
  const { dueCount, kanaDue, kanaRow, focus, empty, loading, error, retry } = useDailyFocus();
  ...
  useEffect(() => {
    ...
    start(focus, dueCount, voice === 'ready', kanaDue, kanaRow).then(
      (ok) => {
        if (!ok) setStartError(true);
      },
      () => setStartError(true),
    );
  }, [loading, error, empty, voice, start, focus, dueCount, kanaDue, kanaRow]);
