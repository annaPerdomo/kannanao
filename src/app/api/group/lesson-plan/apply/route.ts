...
export async function POST(req: NextRequest) {
  ...
  logger.info('Lesson plan applied', {
    ...
    kanaAssigned: kana.assigned.length,
    kanaFailed: kana.failed.length,
  });
...
async function createDeck(args: {
  ...
  const cardRows = (deck.cards ?? []).map((card, i) => ({
    ...
    image_query: (card.imageQuery ?? '').slice(0, PLAN_TEXT_MAX),
    image_url: typeof card.imageUrl === 'string' && card.imageUrl ? card.imageUrl : null,
